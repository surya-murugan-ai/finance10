...
async function extractRealDataFromDocument(doc: any, docType: string) {
  try {
    ...
    // Convert to JSON with raw: false to preserve original string values
    const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false });
    
    // Also get raw cell values for better amount parsing
    const rawCells = {}; // Store raw cell values
    ...
    for (let R = range.s.r; R <= range.e.r; ++R) {
      ...
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        if (cell) {
          row[C] = cell.w || cell.v; // w = formatted value (with Dr/Cr), v = raw value
        } else {
          row[C] = null;
        }
      }
    ...
    console.log(`Routing to extraction for docType: ${docType}, fileName: ${doc.fileName}`);
    switch (docType) {
      case 'sales_register':
      ...
      case 'bank_statement':
        console.log(`About to call extractBankStatementData for ${doc.fileName}`);
        return await extractBankStatementData(jsonData, doc.fileName, rawCells);
...
async function extractPurchaseRegisterData(jsonData: any[], fileName: string) {
  ...
  // For purchase register, data starts at row 4 (after headers at row 3)
  // Row 3 contains: Date, Particulars, Voucher Type, Voucher No., Narration, Value, etc.
  ...
  for (let i = dataStartRow; i < jsonData.length; i++) {
    ...
    if (row && Array.isArray(row) && row.length > 0) {
      ...
      const particulars = row[1] || ''; // Particulars (vendor name)
      const voucherType = row[2] || ''; // Voucher Type
      const voucherNo = row[3] || ''; // Voucher No.
      const narration = row[4] || ''; // Narration
      const value = parseFloat(row[5]?.toString().replace(/[^\d.-]/g, '') || '0'); // Value
      const grossTotal = parseFloat(row[7]?.toString().replace(/[^\d.-]/g, '') || '0'); // Gross Total
      
      // Only add if there's actual data (not empty row)
      if (date && particulars && value > 0) {
        purchases.push({
          purchaseOrder: voucherNo || `PO-${i}`,
          vendorName: particulars,
          purchaseDate: date,
          itemDescription: narration || 'Purchase transaction',
          quantity: 1,
          amount: value,
  ...
  console.log(`Extracted ${purchases.length} purchase transactions from ${fileName}`);
  return { purchases };
}

// Extract bank statement data from Excel
async function extractBankStatementData(jsonData: any[], fileName: string, rawCells: any = null) {
  const transactions = [];
  
  console.log(`Starting bank statement extraction for ${fileName}. Total rows: ${jsonData.length}`);
  
  // For bank statement, data starts at row 6 (after headers at row 5)
  // Row 5 contains: Date, Particulars, Voucher Type, Voucher No., Narration, Value, Gross Total, etc.
  ...
  for (let i = dataStartRow; i < jsonData.length; i++) {
    ...
    if (row && Array.isArray(row) && row.length > 0) {
      const date = row[0] || ''; // Date column
      const particulars = (rawCells && rawCells[i] ? rawCells[i][1] : row[1]) || ''; // Particulars (party name)
      const voucherType = row[2] || ''; // Voucher Type
      const voucherNo = row[3] || ''; // Voucher No.
      const narration = row[4] || ''; // Narration
      const value = row[5]; // Value column
      const grossTotal = row[6]; // Gross Total column
      
      // Parse amounts - look for "Dr" and "Cr" suffixes across all columns
      let debit = 0;
      let credit = 0;
      
      // Check all columns from index 5 onwards for amounts
      // Use raw cell values if available, otherwise use jsonData
      const rowData = rawCells && rawCells[i] ? rawCells[i] : row;
      
      for (let colIndex = 5; colIndex < Math.max(row.length, rowData.length || 0); colIndex++) {
        const cellValue = rowData[colIndex] || row[colIndex];
        // Debug logging disabled - extraction working correctly
        if (cellValue != null && typeof cellValue === 'string' && cellValue.trim()) {
          const amount = parseFloat(cellValue.toString().replace(/[^\d.-]/g, '') || '0');
          if (amount > 0) {
            if (cellValue.includes('Dr')) {
              debit += amount;
            } else if (cellValue.includes('Cr')) {
              credit += amount;
            }
          }
        }
      ...
      if (date && particulars && (debit > 0 || credit > 0)) {
        transactions.push({
          ...
          reference: `${voucherType}-${voucherNo}`,
          debit: debit,
          credit: credit,
          balance: 0, // Balance calculation would need running total
          narration: narration || '',
  ...
  console.log(`Extracted ${transactions.length} bank transactions from ${fileName}`);
  return { transactions };
}
...
async function generateProfitLoss(journalEntries: any[]): Promise<any> {
  ...
  for (const [code, balance] of accountBalances) {
    if (code.startsWith('4')) {
      ...
      const netRevenue = -balance.netBalance; // Negative of net balance gives credit balance
      if (netRevenue > 0) {
        revenue.push({
          accountCode: code,
      ...
      // Expense accounts (5xxx) - classify ALL 5xxx accounts as expenses
      const netExpense = Math.abs(balance.netBalance); // Use absolute value for consistency
      if (netExpense > 0) {
        expenses.push({
          accountCode: code,
          accountName: balance.accountName,
...
async function generateBalanceSheet(journalEntries: any[]): Promise<any> {
  ...
  for (const [code, balance] of accountBalances) {
    if (code.startsWith('1')) {
      ...
      const amount = balance.netBalance; // Positive net balance is debit balance
      if (amount > 0) {
        assets.push({
          accountCode: code,
          accountName: balance.accountName,
